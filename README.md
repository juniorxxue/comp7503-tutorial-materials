...
## Recommended release process for the course team

For the smoothest student experience:

1. Test this stack once on macOS, Windows, and Linux.
2. Pre-build and push the custom Node-RED image to Docker Hub or GHCR.
3. Change Compose from `build:` to `image:` before release.
4. Tell students to run `docker compose pull && docker compose up -d`.

That turns setup into a pull-and-run workflow.

## Maintainer note about the MongoDB Node-RED package

This starter keeps `node-red-contrib-mongodb3` because that matches the existing tutorial material. During lockfile generation, `npm audit` reported unresolved high-severity issues in one of its transitive dependencies. If you are updating the course materials anyway, it is worth testing the maintained `node-red-node-mongodb` package and standardizing on that instead.

## Optional fallback for students with problematic laptops

If you want a near-zero local setup path, publish the same repository with a dev container or GitHub Codespaces configuration. Then students open the repo in a browser-backed environment instead of installing a full local toolchain.
